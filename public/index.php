...
<body>
    ...
    $pdo = conectar();
    $sent = $pdo->query("SELECT * FROM albumes ORDER BY titulo");
    ?>
            ...
            <main class="flex-1 grid grid-cols-3 gap-4 justify-center justify-items-center">
                <?php foreach ($sent as $fila) : ?>
                    <div class="p-6 max-w-xs min-w-full bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                        <!-- Titulo -->
                        <a href="#">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><?= hh($fila['titulo']) ?></h5>
                        </a>
                        <!-- Precio -->
                        ...
                        <table class="mx-auto text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <?php foreach ($carrito->getLineas() as $id => $linea) : ?>
                                    <?php
                                    $album = $linea->getAlbum();
                                    $cantidad = $linea->getCantidad();
                                    ?>
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        <td class="py-4 px-6"><?= $album->getTitulo() ?></td>
